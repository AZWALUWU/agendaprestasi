...
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = 'public' AND tablename = 'tryout_subject_timers') THEN
    DROP POLICY IF EXISTS "Users manage own timers" ON public.tryout_subject_timers;
    DROP TABLE public.tryout_subject_timers CASCADE;
  END IF;

  IF EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = 'public' AND tablename = 'tryout_answers') THEN
    DROP POLICY IF EXISTS "Users manage own answers" ON public.tryout_answers;
  ...
  IF EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = 'public' AND tablename = 'tryout_sessions') THEN
    DROP POLICY IF EXISTS "Users manage own sessions" ON public.tryout_sessions;
    DROP POLICY IF EXISTS "Users read all submitted sessions for ranking" ON public.tryout_sessions;
    DROP TABLE public.tryout_sessions CASCADE;
  END IF;

  IF EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = 'public' AND tablename = 'tryout_event_questions') THEN
    DROP POLICY IF EXISTS "Public read event questions metadata" ON public.tryout_event_questions;
    DROP POLICY IF EXISTS "Admin manage event questions" ON public.tryout_event_questions;
    DROP TABLE public.tryout_event_questions CASCADE;
  END IF;

  IF EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = 'public' AND tablename = 'tryout_events') THEN
    DROP TRIGGER IF EXISTS update_tryout_events_updated_at ON public.tryout_events;
    DROP POLICY IF EXISTS "Public read published events" ON public.tryout_events;
    DROP POLICY IF EXISTS "Admin manage events" ON public.tryout_events;
...
-- Drop question-images storage policies only (bucket dihapus via Storage API, bukan SQL)
...
-- ============================================================================
-- CLEANUP: Drop existing policies to recreate fresh
-- ============================================================================
...
FOR INSERT
WITH CHECK (
  public.has_user_role(auth.uid(), ARRAY['super_admin'])
);
...
CREATE TABLE IF NOT EXISTS public.posts (
  ...
  category          TEXT        NOT NULL
                      CHECK (category IN ('scholarship', 'competition', 'event')),
  deadline          DATE,
  open_date         DATE,
...
-- Fix category constraint jika tabel sudah ada sebelumnya
...
USING (
  status = 'published'
  OR public.has_user_role(auth.uid(), ARRAY['admin', 'super_admin'])
);
...
USING (
  (
    author_id = auth.uid()
    AND public.has_user_role(auth.uid(), ARRAY['admin'])
  )
